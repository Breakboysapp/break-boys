/**
 * One-off seed: add 2025 Bowman Baseball (flagship) with its Beckett
 * checklist. Mirrors seed-2023-bowman.ts but scoped to just this one product
 * so re-running never touches any other catalog entry (e.g. it leaves the
 * existing 2025 Bowman Draft Baseball untouched). The only delete is scoped
 * to this product's own cards before re-inserting, so it's idempotent.
 *
 *   npx tsx scripts/seed-2025-bowman.ts
 */
...
const PRODUCTS: Array<{
...
}> = [
  {
    ...
    sport: "MLB",
    slug: "2025-bowman-baseball-cards",

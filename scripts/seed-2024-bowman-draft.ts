/**
 * One-off seed: add 2024 Bowman Draft Baseball with its Beckett checklist.
 * Mirrors seed-2023-bowman.ts but scoped to just this one product so
 * re-running never touches any other catalog entry — in particular it leaves
 * the existing 2025 Bowman Draft Baseball (7,131 cards) untouched. The only
 * delete is scoped to this product's own cards before re-inserting, so it's
 * idempotent.
 *
 *   npx tsx scripts/seed-2024-bowman-draft.ts
 */
...
const PRODUCTS: Array<{
...
}> = [
  {
    ...
    sport: "MLB",
    slug: "2024-bowman-draft-baseball-cards",

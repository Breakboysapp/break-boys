...
export default async function HotSportPage({
  ...
  // Composite "Chase Score" = log(30d sales) × log(30d cents).
  //
  // Same shape as the Hot Score formula but on the 30-day window
  // — surfaces players whose cards have SUSTAINED market presence
  // (consistent multi-week dollar flow + transaction count), not
  // just this week's spike. Distinguishes the standing market
  // hierarchy ("biggest chases") from the right-now movement
  // ("on fire").
  //
  // Log compression + multiplication still gates: a player with a
  // single $50k sale in a month doesn't dominate, and a player who
  // moved 10,000 base cards at $1 doesn't either. Both signals must
  // be high.
  const chaseScore = (sales: number, cents: number) => {
  ...
  const ranked = snapshots
    ...
    .map((s) => ({
      ...
      chaseScore: chaseScore(s.last30dSales, s.last30dCents),
    }))
    .filter((r) => r.chaseScore > 0)
      ...
      {ranked.length === 0 ? (
        ...
        <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
          <div className="grid grid-cols-12 gap-3 border-b border-slate-200 bg-ink px-4 py-2.5 text-[10px] font-bold uppercase tracking-tight-2 text-white">
            ...
            <div className="col-span-3 sm:col-span-2 text-right">30d sales</div>
            <div
              className="col-span-3 sm:col-span-2 text-right"
              title="Chase Score blends 30-day sales count × 30-day dollar volume — players whose cards command sustained market presence, not just a single-week spike. Both signals must be high. Big number is the 30-day dollar flow; smaller below is current-week activity for context."
            >
              30d $
            </div>
          </div>
              ...
              >
                ...
                <div className="col-span-3 sm:col-span-2 text-right tabular-nums text-xs">
                  <div className="font-bold text-ink">
                    {r.last30dSales.toLocaleString()}
                  </div>
                  <div className="text-[10px] font-medium text-slate-400">
                    7d {r.currentWeekSales.toLocaleString()}
                  </div>
                </div>
                <div className="col-span-3 sm:col-span-2 text-right tabular-nums">
                  <div className="text-sm font-extrabold text-ink">
                    {formatUsd(r.last30dCents)}
                  </div>
                  <div className="text-[10px] font-medium text-slate-400">
                    7d {formatUsd(r.currentWeekCents)}
                  </div>
                </div>
              </li>
            ))}
